...
export default {
  ...
  data() {
    return {
      activeFormType: 'element', // 默认使用PC-UI (Element)
      activeTab: 'formData', // 默认显示表单数据标签
      schema: jsonSchema, // 预备用工具方法生成
      formData: JSON.parse(JSON.stringify(formData)), // 克隆数据，避免直接修改原始数据
      originalFormData: JSON.parse(JSON.stringify(formData)), // 保存原始数据用于重置
      formPropsPC: {
